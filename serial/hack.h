 ...
 */
 ...
 static inline aclError real_aclrtMemset(void *devPtr, size_t maxCount, int32_t value, size_t count)
{
    using func_ptr = aclError (*)(void *, size_t, int32_t, size_t);
    static auto func_entry = reinterpret_cast<func_ptr>(get_symbol_acl("aclrtMemset"));
    if (!func_entry) return ACL_ERROR_LD_SO_NOT_LOADED;
    return func_entry(devPtr, maxCount, value, count);
}

// hack.h

// [修改] 使用 Async 版本以支持流同步
static inline aclError real_aclrtMemsetAsync(void *devPtr, size_t maxCount, int32_t value, size_t count, aclrtStream stream)
{
    using func_ptr = aclError (*)(void *, size_t, int32_t, size_t, aclrtStream);
    static auto func_entry = reinterpret_cast<func_ptr>(get_symbol_acl("aclrtMemsetAsync"));
    if (!func_entry) return ACL_ERROR_LD_SO_NOT_LOADED;
    return func_entry(devPtr, maxCount, value, count, stream);
}

 static inline aclError real_aclrtFreeHost(void *ptr)
{
    using func_ptr = aclError (*)(void *);
    static auto func_entry = reinterpret_cast<func_ptr>(get_symbol_acl("aclrtFreeHost"));
    if (!func_entry) return ACL_ERROR_LD_SO_NOT_LOADED;
 ...
 static inline aclError real_aclrtDestroyStream(aclrtStream stream) {
    using func_ptr = aclError (*)(aclrtStream);
    static auto func_entry = reinterpret_cast<func_ptr>(get_symbol_acl("aclrtDestroyStream"));
    if (!func_entry) return ACL_ERROR_LD_SO_NOT_LOADED;
    return func_entry(stream);
}
