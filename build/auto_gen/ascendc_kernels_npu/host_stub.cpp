...
struct ascend_kernels {
    ...
    uint8_t aiv_buf[10632];
} __ascend_kernel_ascend910b3_ascendc_kernels_npu __attribute__ ((section (".ascend.kernel.ascend910b3.ascendc_kernels_npu"))) = {1,1,1,10632,10632,{0}};
...
uint32_t launch_and_profiling_aes_cipher_kernel_npu(uint64_t func_key, uint32_t blockDim, void* stream, void **args, uint32_t size)
{
    uint64_t startTime;
    const char *name = "aes_cipher_kernel_npu";
...
{
    struct {
        alignas(((alignof(void*) + 3) >> 2) << 2) void* input_x;
        alignas(((alignof(void*) + 3) >> 2) << 2) void* key;
        alignas(((alignof(void*) + 3) >> 2) << 2) void* iv;
        alignas(((alignof(void*) + 3) >> 2) << 2) void* output;
        alignas(((alignof(uint32_t) + 3) >> 2) << 2) uint32_t totalLength;
        alignas(((alignof(uint32_t) + 3) >> 2) << 2) uint32_t mode;
    ...
    __ascendc_args.input_x = input_x;
    __ascendc_args.key = key;
    __ascendc_args.iv = iv;
    __ascendc_args.output = output;
    __ascendc_args.totalLength = totalLength;
    __ascendc_args.mode = mode;

    __ascendc_ret = launch_and_profiling_aes_cipher_kernel_npu(0, blockDim, stream, (void **)&__ascendc_args, sizeof(__ascendc_args));
